...
public class registrarTratamiento extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        long documento = Long.parseLong(request.getParameter("IDT"));
        String nombre = request.getParameter("NOMT");
        int dir = Integer.parseInt(request.getParameter("DURA"));
        tratamiento tra=new tratamiento();
        try (PrintWriter out = response.getWriter()) {
            if(!tra.existe(documento)) {
                if (tra.registrar(documento, nombre, dir)) {
