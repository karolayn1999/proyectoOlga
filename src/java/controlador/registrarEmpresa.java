...
public class registrarEmpresa extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        long documento = Long.parseLong(request.getParameter("NIT"));
        String nombre = request.getParameter("NOMBREE");
        String dir = request.getParameter("DIR");
        long celular = Long.parseLong(request.getParameter("TEL"));
        empresa em=new empresa();
        try (PrintWriter out = response.getWriter()) {
            if(!em.existe(documento)) {
                if (em.registrar(documento, nombre, dir, celular)) {
